...
interface EditUserFlowModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId: string;
  node: {
...
export const EditUserFlowModal = ({
  isOpen,
  onClose,
  projectId,
  node,
  onNodeUpdated,
}: EditUserFlowModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && node) {
      setFormData({
        flow_name: node.metadata?.flow_name || node.title,
        description: node.metadata?.description || "",
        start_state: node.metadata?.start_state || "",
        end_state: node.metadata?.end_state || "",
        steps: node.metadata?.steps && node.metadata.steps.length >= 2 ? node.metadata.steps : ["", ""],
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success(`User flow "${validated.flow_name}" updated`);
      ...
      if (error instanceof z.ZodError) {
        ...
        toast.error('Failed to update user flow');
      }
    } finally {
