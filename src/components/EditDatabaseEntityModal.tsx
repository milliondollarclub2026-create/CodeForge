...
interface EditDatabaseEntityModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId: string;
  node: {
...
export const EditDatabaseEntityModal = ({
  isOpen,
  onClose,
  projectId,
  node,
  onNodeUpdated,
}: EditDatabaseEntityModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && node) {
      setFormData({
        entity_name: node.metadata?.entity_name || node.title,
        description: node.metadata?.description || "",
        security_model: node.metadata?.security_model || "private_owner",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Verify node ownership through project
      ...
      if (!nodeCheck || (nodeCheck.projects as any).user_id !== user.id) {
      ...
      toast.success(`Database entity "${validated.entity_name}" updated`);
      ...
      if (error instanceof z.ZodError) {
        ...
        console.error('Error updating database entity:', error);
        toast.error('Failed to update database entity');
      }
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-slate-900 border-slate-800 text-white">
        <DialogHeader>
          <DialogTitle>Edit Database Entity</DialogTitle>
        </DialogHeader>
